...
const AccountPage = () => {
  ...
  const fetchUserTeams = async (teams_id) => {
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8080/api/teams/pokemon/${teams_id}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const deleteTeam = async () => {
    if (selectedTeam !== null) {
      try {
        const response = await fetch(`http://localhost:8080/api/teams/pokemon/${selectedTeam}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
        ...
        const updatedUserTeams = user.teams.filter((team) => team.id !== selectedTeam);
        setUser((prevUser) => ({
          ...prevUser,
            ...
            {user.teams.map((team) => (
              <div key={team.id}>
                <div className="team-names">
                  {team.team_name}
                  <button onClick={() => showTeam(team.id)}>Show team info</button>
                </div>
              </div>
            ))}
          </div>
            ...
            {teamDetails && (
              <div className="teams-container">
                ...
                <div className="pokemon-list">
                  {teamDetails.map((team) => (
                    <div key={team.id} className="team-details">
                      <div className="TeamPokemon-details">
                        {team.pokemon.sprite && (
                          <img src={team.pokemon.sprite} alt={team.pokemon.name} />
                        )}
                        <div className="pokemon-name">
                          {team.pokemon.name.charAt(0).toUpperCase() + team.pokemon.name.slice(1)}
                        </div>
                        <div className="types">
                          <p
                            className={
                          ...
                          >
                            {team.pokemon.type1 && team.pokemon.type1.charAt(0).toUpperCase() + team.pokemon.type1.slice(1)}
                          </p>
                          <p
                            className={
                              team.pokemon.type2
                          ...
                          >
                            {team.pokemon.type2 && team.pokemon.type2.charAt(0).toUpperCase() + team.pokemon.type2.slice(1)}
                          </p>
                      ...
                      <div className="TeamPokemon-moveset">
                        <div className="TeamPokemonleft-moves">
                          {team.move1 && <p>{team.move1.charAt(0).toUpperCase() + team.move1.slice(1)}</p>}
                          {team.move2 && <p>{team.move2.charAt(0).toUpperCase() + team.move2.slice(1)}</p>}
                        </div>
                        <div className="Teampokemonright-moves">
                          {team.move3 && <p>{team.move3.charAt(0).toUpperCase() + team.move3.slice(1)}</p>}
                          {team.move4 && <p>{team.move4.charAt(0).toUpperCase() + team.move4.slice(1)}</p>}
                        </div>
                  ...
                  <button onClick={clearSelectedTeam} className="Return-button">Return</button>
                </div>
                
                
                <div className="side-picture right-picture">
