...
teamsRouter.post("/", async (req, res, next) => {
  try {
    const { user_id, gen, pokemon1, pokemon2, pokemon3, pokemon4, pokemon5, pokemon6 } = req.body; 
    const addTeam = await prisma.teams.create({
      data: { user_id, gen, pokemon1, pokemon2, pokemon3, pokemon4, pokemon5, pokemon6 }
    });
    res.send(addTeam);
  } catch (err) {
    next(err);
